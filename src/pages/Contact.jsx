import { useState } from "react";
import { Helmet } from "react-helmet-async";
import PageWrapper from "../components/PageWrapper";
import SectionHeading from "../components/SectionHeading";
import Toast from "../components/Toast";
import siteConfig from "../data/siteConfig";

const initial = {
    name: "",
    phone: "",
    city: "",
    serviceType: "",
    monthlyBill: "",
    message: "",
};

export default function Contact() {
    const [form, setForm] = useState(initial);
    const [toast, setToast] = useState("");
    const [errors, setErrors] = useState({});

    const handleChange = (e) =>
        setForm((p) => ({ ...p, [e.target.name]: e.target.value }));

    const validate = () => {
        const e = {};
        if (!form.name.trim()) e.name = "Name is required";
        if (!form.phone.trim()) e.phone = "Phone is required";
        if (!form.city.trim()) e.city = "City is required";
        if (!form.serviceType) e.serviceType = "Select a service";
        if (!form.monthlyBill) e.monthlyBill = "Select bill range";
        setErrors(e);
        return Object.keys(e).length === 0;
    };

    const submit = (ev) => {
        ev.preventDefault();
        if (!validate()) return;

        const text =
            `*New Solar Enquiry*%0A` +
            `Name: ${encodeURIComponent(form.name)}%0A` +
            `Phone: ${encodeURIComponent(form.phone)}%0A` +
            `City: ${encodeURIComponent(form.city)}%0A` +
            `Service: ${encodeURIComponent(form.serviceType)}%0A` +
            `Monthly Bill: ${encodeURIComponent(form.monthlyBill)}%0A` +
            `Message: ${encodeURIComponent(form.message || "N/A")}`;

        setToast("Opening WhatsApp...");
        setTimeout(() => setToast(""), 2000);

        window.open(
            `https://wa.me/${siteConfig.whatsappNumber}?text=${text}`,
            "_blank",
            "noopener,noreferrer"
        );

        setForm(initial);
        setErrors({});
    };

    // Tailwind input style (works great on mobile)
    const field =
        "w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-green-500 focus:ring-2 focus:ring-green-500/20 dark:border-slate-800 dark:bg-slate-950";
    const errorText = "text-xs text-red-500 -mt-2";

    return (
        <PageWrapper>
            <Helmet>
                <title>Contact | SunPeak Solar</title>
                <meta
                    name="description"
                    content="Contact SunPeak Solar for free consultation and quote."
                />
            </Helmet>

            <Toast message={toast} />

            <section className="container-app py-10 sm:py-16">
                <SectionHeading title="Contact Us" subtitle="Get free consultation and quote." />

                <div className="mt-8 grid gap-6 lg:grid-cols-2">
                    {/* FORM */}
                    <form className="card p-5 sm:p-6 space-y-4" onSubmit={submit}>
                        <div className="space-y-2">
                            <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>
                                Name*
                            </label>
                            <input
                                className={field}
                                name="name"
                                placeholder="Your name"
                                value={form.name}
                                onChange={handleChange}
                                aria-label="Name"
                            />
                            {errors.name && <p className={errorText}>{errors.name}</p>}
                        </div>

                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                                <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>
                                    Phone*
                                </label>
                                <input
                                    className={field}
                                    name="phone"
                                    placeholder="Phone number"
                                    value={form.phone}
                                    onChange={handleChange}
                                    aria-label="Phone"
                                    inputMode="tel"
                                />
                                {errors.phone && <p className={errorText}>{errors.phone}</p>}
                            </div>

                            <div className="space-y-2">
                                <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>
                                    City*
                                </label>
                                <input
                                    className={field}
                                    name="city"
                                    placeholder="City"
                                    value={form.city}
                                    onChange={handleChange}
                                    aria-label="City"
                                />
                                {errors.city && <p className={errorText}>{errors.city}</p>}
                            </div>
                        </div>

                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                                <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>
                                    Service Type*
                                </label>
                                <select
                                    className={field}
                                    name="serviceType"
                                    value={form.serviceType}
                                    onChange={handleChange}
                                    aria-label="Service Type"
                                >
                                    <option value="">Select service</option>
                                    <option>Residential Solar</option>
                                    <option>Commercial Solar</option>
                                    <option>Rooftop Solar</option>
                                    <option>Solar Maintenance / AMC</option>
                                    <option>Solar Water Heater</option>
                                </select>
                                {errors.serviceType && <p className={errorText}>{errors.serviceType}</p>}
                            </div>

                            <div className="space-y-2">
                                <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>
                                    Monthly Electricity Bill*
                                </label>
                                <select
                                    className={field}
                                    name="monthlyBill"
                                    value={form.monthlyBill}
                                    onChange={handleChange}
                                    aria-label="Monthly electricity bill"
                                >
                                    <option value="">Select range</option>
                                    <option>Below ₹2000</option>
                                    <option>₹2000 - ₹5000</option>
                                    <option>₹5000 - ₹10000</option>
                                    <option>Above ₹10000</option>
                                </select>
                                {errors.monthlyBill && <p className={errorText}>{errors.monthlyBill}</p>}
                            </div>
                        </div>

                        <div className="space-y-2">
                            <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>
                                Message (optional)
                            </label>
                            <textarea
                                className={field}
                                name="message"
                                rows={4}
                                placeholder="Write your requirement..."
                                value={form.message}
                                onChange={handleChange}
                                aria-label="Message"
                            />
                        </div>

                        <button type="submit" className="btn-primary w-full">
                            Request Quote on WhatsApp
                        </button>

                        <p className="text-xs text-center" style={{ color: "var(--muted)" }}>
                            By submitting, you agree to be contacted via WhatsApp/Call.
                        </p>
                    </form>

                    {/* INFO + MAP */}
                    <div className="space-y-4">
                        <div className="card p-5">
                            <h3 className="text-base font-semibold">Find Us</h3>

                            <div className="mt-3 space-y-2 text-sm">
                                <p style={{ color: "var(--muted)" }}>{siteConfig.address}</p>
                                <p>Hours: Mon–Sat, 9:30 AM – 6:30 PM</p>
                                <p>
                                    Phone:{" "}
                                    <a className="text-green-600 dark:text-green-400 hover:underline" href={`tel:${siteConfig.phoneNumber}`}>
                                        {siteConfig.phoneNumber}
                                    </a>
                                </p>
                                <p>
                                    Email:{" "}
                                    <a className="text-green-600 dark:text-green-400 hover:underline" href={`mailto:${siteConfig.email}`}>
                                        {siteConfig.email}
                                    </a>
                                </p>
                            </div>

                            <div className="mt-4 flex flex-col sm:flex-row gap-3">
                                <a
                                    href={siteConfig.googleMapDirectionsUrl}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="btn-secondary text-center"
                                >
                                    Get Directions
                                </a>

                                <a
                                    href={`https://wa.me/${siteConfig.whatsappNumber}`}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="btn-primary text-center"
                                >
                                    WhatsApp Now
                                </a>
                            </div>
                        </div>

                        {/* RESPONSIVE MAP (mobile fixed) */}
                        <div className="card overflow-hidden">
                            <div className="relative w-full aspect-[16/10] sm:aspect-[16/9]">
                                <iframe
                                    className="absolute inset-0 h-full w-full border-0"
                                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3539.391284626702!2d77.6586994754596!3d27.488204076308847!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x397371bb1362c5b3%3A0x3eb0c4d6e5b25a0b!2sMathura%20Best%20properties%20sales%20%26%20purchase%20pratap%20farm%20dealer!5e0!3m2!1sen!2sin!4v1771737358383!5m2!1sen!2sin"
                                    loading="lazy"
                                    allowFullScreen
                                    referrerPolicy="no-referrer-when-downgrade"
                                    title="Google Map"
                                />
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </PageWrapper>
    );
}